...
chown root.sdcard_rw $storagePath/bugreport_$timestamp.txt

# Forward info to the BugToGo app
/system/bin/am start -a motorola.intent.action.BUGTOGO -t "application/octet-stream" \
    -e "path" "$storagePath/bugreport_$timestamp.txt" \
    -e "serial" "$serialNum" \
